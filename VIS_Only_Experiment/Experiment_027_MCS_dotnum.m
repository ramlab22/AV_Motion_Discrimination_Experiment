...
while (BreakState ~= 1) 
    ...
    dataout(output_counter,1:11) = {'Trial #' 'Position #' 'Fixation Correct' 'Visual Reward' 'Catch Trial' 'Target Correct' 'Total Trial Time (sec)' 'DotNum' 'Direction of Motion' 'Incorrect Target Fixation' 'Stimulus Modality'}; %Initialize Columns for data output cell
    ...
    while (trialcounter <= total_trials) && (BreakState ~= 1) % each trial
        ...
        %Inititlize the dot num and direction for each trial
        dotInfo.dir = dotInfo.random_dir_list(trialcounter); %randomly choose 0 or 1 for dir
        dotInfo.dotnum = dotInfo.random_dotnum_list(trialcounter);% Random dotnum for each trial
...
%         [adjust_right, adjust_left] = targ_adjust(pos);%Outputs the adjustments in pixels for dR and dL equations later on 
        ...
        if fix_timeout ~= 1
            % Draw the RDK
            [rdk_timeout, eye_data_matrix] = RDK_Draw_dotnum(ExpInfo, dotInfo, window, xCenter, yCenter, h_voltage, k_voltage, TDT, start_block_time, eye_data_matrix, trialcounter, fix_point_color);
            if rdk_timeout ~= 1
               rdk_reward = 'Yes'; 
               if baron_fixation_training==1 
        ...
        dataout(output_counter,1:11) = {trialcounter pos fix_reward rdk_reward catchtrial target_reward trial_time dotInfo.dotnum dotInfo.dir incorrect_target_fixation stim_modality}; 
...
[Fixation_Success_Rate, RDK_Success_Rate, Target_Success_Rate_Regular, Target_Success_Rate_Catch] = SR_CALC(dataout,total_trials,num_regular_trials,num_catch_trials)
    ...
    %Count how many rew and N/A per dotnum 
    dotInfo.dotnumFreq = dotnumFreq_finder(dataout, dotInfo);
    prob_correct = dotnum_probability(dataout,dotInfo)
    prob_zero = prob_correct(1,:); 
    
    [Right_dataout, Left_dataout] = direction_splitter(dataout);
    
    dotInfo.dotnumFreq_right = dotnumFreq_finder(Right_dataout, dotInfo);
    dotInfo.dotnumFreq_left = dotnumFreq_finder(Left_dataout, dotInfo);
    
    probcorrect_Righttrials = directional_probability_visual_dotnum(Right_dataout, dotInfo); 
