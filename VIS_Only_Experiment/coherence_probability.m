function [prob] = coherence_probability(dataout, dotInfo) 
    ...
    for i_coherence = 1:length(coherences)
        for v = 1:length(dataout)
            if (strcmp(dataout{v,6},'Yes')) && (dataout{v,8} == coherences(i_coherence))  && (strcmp(dataout{v,5},'No')) %Target Reward & Not a Catch Trial, Correct Rightward or Leftward Response
