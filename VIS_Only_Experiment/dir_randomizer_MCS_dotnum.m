function [random_dir_list] = dir_randomizer_MCS_dotnum(ExpInfo,dotInfo)
...
%locations and catch trial(1 coherence) locations
    ...
    for dotnum = dotInfo.dotnumSet

        if length(dotInfo.dirSet) > 1 %If we have more than 1 direction to choose from

            numberOfElements = sum(dotInfo.random_dotnum_list == dotnum); %Sums all of the same coherences from the random list 
            ...
            indeces = find(dotInfo.random_dotnum_list == dotnum); 
            
            for i = 1:numberOfElements
                if signal(i) == 0
                     random_dir_list(indeces(i)) = dotInfo.dirSet(2); %Leftward wherever that dotnum appears 
                elseif signal(i) == 1
                    random_dir_list(indeces(i)) = dotInfo.dirSet(1); %Rightward wherever that dotnum appears 
                end
