function [rdk_timeout, eye_data_matrix] = RDK_Draw_dotnum(ExpInfo, dotInfo, curWindow, xCenter, yCenter, h_voltage, k_voltage, TDT, start_block_time, eye_data_matrix, trial, fix_point_color)
...
refresh_rate = 1/ifi; 
    ...
    ndots = min(dotInfo.dotnum,ceil(50 * apD .* apD * 0.01 / refresh_rate));
...
%         -sin(pi*dotInfo.dir(df)/180.0)], ndots(df),1);    
     dxdy{df} = repmat((dotInfo.speed(df)/10) * (10/apD(df)) * ...
         (1/refresh_rate) * [cos(pi*dotInfo.dir(df)/180.0), ...
         -sin(pi*dotInfo.dir(df)/180.0)], ndots(df),1);    
