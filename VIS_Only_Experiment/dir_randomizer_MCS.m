...
%locations and catch trial(1 coherence) locations
    ...
    for c = dotInfo.coherences
        ...
        if length(dotInfo.dirSet) > 1 %If we have more than 1 direction to choose from
            ...
            numberOfElements = sum(dotInfo.random_coh_list == c); %Sums all of the same coherences from the random list 
            ...
            indeces = find(dotInfo.random_coh_list == c); 
            
            for i = 1:numberOfElements
                if signal(i) == 0
                     random_dir_list(indeces(i)) = dotInfo.dirSet(2); %Leftward wherever that coherence appears 
                elseif signal(i) == 1
                    random_dir_list(indeces(i)) = dotInfo.dirSet(1); %Rightward wherever that coherence appears 
                end
