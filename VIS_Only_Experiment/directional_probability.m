function [prob] = directional_probability(directional_dataout, dotInfo) 
    ...
    for i_coherence = 1:length(coherences)
        for v = 1:length(directional_dataout)
    ...
    for c = 1:length(dotInfo.coherences)
        coherence_success_rate(2,c) = coherence_rew_numbers(2,c)/((round(dotInfo.cohFreq(c)/2))-coherence_rew_numbers(3,c));  %Subtract the trials where there was no chance for reward(N/A Target Correct)
    end
