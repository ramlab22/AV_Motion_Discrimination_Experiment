function [n_trials_with_response,n_trials_with_reward,proportion_response_reversals_after_correct_response,proportion_response_reversals_after_incorrect_response] = response_reversal_proportions2_visual(dataout)
dataout_columnlabels=dataout(1,:);

motion_directions=cell2mat(dataout(2:end,9)); %get list of motion directions, 0(Right) or 180(left)
...
for i_trial = 1:n_completedtrials
    ...
    if motion_directions(i_trial,1) == 180 & response_accuracy{i_trial,1}(2) == 'e'
        response_direction(i_trial,1) = 0;
    end
    if motion_directions(i_trial,1) == 180 & response_accuracy{i_trial,1}(2) == 'o'
        response_direction(i_trial,1) = 1;
    end
end
