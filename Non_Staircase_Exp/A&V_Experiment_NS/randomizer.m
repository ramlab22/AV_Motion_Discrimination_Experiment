...
%locations and catch trial(1 coherence) locations
    ...
    %direction from ExpInfo.dir_set, Currently only works for L and R
    ...
    for c = ExpInfo.coherences 

        if length(dotInfo.dirSet) > 1 %If we have more than 1 direction to choose from

            numberOfElements = sum(ExpInfo.cohSet == c); %Sums all of the same coherences from the random list cohSet
