function [prob] = coherence_probability_vis(dataout, dotInfo) 
...
coherence_rew_numbers = [dotInfo.coherences;
    ...
    for i_coherence = 1:length(coherences)
        for v = 1:length(dataout(:,1))
            if (strcmp(dataout{v,6},'Yes')) && (dataout{v,11} == coherences(i_coherence))  && (strcmp(dataout{v,5},'No')) %Target Reward & Not a Catch Trial
                coherence_rew_numbers(2,i_coherence) = coherence_rew_numbers(2,i_coherence)+1;
            end
            if  (strcmp(dataout{v,6},'N/A')) && (dataout{v,11} == coherences(i_coherence)) && (strcmp(dataout{v,5},'No')) %No chance for target reward & Not a catch trial
