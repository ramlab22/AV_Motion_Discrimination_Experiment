function [fitthresh,deltathresh,PsychSlope] = createFitMF(HR,PC,threshold,lvlup,flipr)
%CREATEFIT(LL,PC)
%  Create a fit.
%
...
opts.Display = 'Off';
opts.Lower = [a 2 0.8 0.2];
%opts.Lower = [a 0.5 1 1]; %for dprime
opts.Upper = [a*2 25 1 0.7];
...
PsychSlope=PsychSlope(1)

fid1 = fopen('DRnRTdata.txt','a+');
fprintf(fid1,'%4.4f\t',PsychSlope);
fclose(fid1);
...
fitthresh=fitthresh(1)
deltathresh=fitthresh-threshold;
deltathresh=deltathresh(1)
