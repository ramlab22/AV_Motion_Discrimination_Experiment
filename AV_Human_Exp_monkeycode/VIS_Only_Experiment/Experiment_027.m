...
%         [adjust_right, adjust_left] = targ_adjust(pos);%Outputs the adjustments in pixels for dR and dL equations later on 
        ...
        if fix_timeout ~= 1 && rdk_timeout ~= 1 && baron_fixation_training ~= 1
            ...
            for frame = 1:target_time_frames - waitframes
                ...
                % function's coordinates are backwards from ours
...
[Fixation_Success_Rate, RDK_Success_Rate, Target_Success_Rate_Regular, Target_Success_Rate_Catch] = SR_CALC(dataout,total_trials,num_regular_trials,num_catch_trials)
    ...
    prob_Rresp_Right = directional_probability(Right_dataout, dotInfo); 
    prob_Rresp_Left = directional_probability(Left_dataout, dotInfo); 
    
    [x, y, fig_both, coeff_p_values] = psychometric_plotter(prob_Rresp_Right,prob_Rresp_Left, dotInfo, save_name);
    
    %%Make Rightward only graph
    prob_Rresp_right_only = coherence_probability_1_direction(Right_dataout, dotInfo);
    [R_coh, R_pc, R_fig] = psychometric_plotter_1_direction(prob_Rresp_right_only, 'RIGHT ONLY', dotInfo, save_name);
    
    %%Make Leftward only graph
    prob_Rresp_left_only = coherence_probability_1_direction(Left_dataout, dotInfo);
    [L_coh, L_pc, L_fig] = psychometric_plotter_1_direction(prob_Rresp_left_only, 'LEFT ONLY', dotInfo, save_name);
