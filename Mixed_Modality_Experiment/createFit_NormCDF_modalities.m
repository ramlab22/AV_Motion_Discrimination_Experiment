function [fig, AUD_p_values, VIS_p_values,...
    AUD_mu, VIS_mu, AUD_std, VIS_std] = createFit_NormCDF_modalities(AUD_coh_list, AUD_pc,...
                                                                   VIS_coh_list, VIS_pc,...
...
legend('AUD','VIS', 'AUD - NormCDF', 'VIS - NormCDF', 'Location', 'Best', 'Interpreter', 'none' );
% Label axes
...
ylim([0 1])
grid on
text(0,.2,"AUD mu: " + AUD_mu)
text(0,.1, "AUD std cum gaussian: " + AUD_std)
text(0,.4,"VIS mu: " + VIS_mu)
text(0,.3, "VIS std cum gaussian: " + VIS_std)
end
