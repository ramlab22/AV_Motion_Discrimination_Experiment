...
coherence_rew_numbers = [AVInfo.coherences_aud;AVInfo.coherences_dot;zeros(1,length(AVInfo.coherences_aud));zeros(1,length(AVInfo.coherences_aud))];
...
if strcmp(direction, "Right")
   ...
   %     coherence_success_rate(3,c) = coherence_rew_numbers(3,c)/((AVInfo.cohFreq_right_aud(2,c))-(coherence_rew_numbers(4,c)));  %Subtract the trials where there was no chance for reward(N/A Target Correct)
        coherence_success_rate(3,c) = coherence_rew_numbers(3,c)/(AVInfo.cohFreq_right_aud(2,c));  %Subtract the trials where there was no chance for reward(N/A Target Correct)
...
elseif strcmp(direction, "Left")
    for c = 1:length(AVInfo.coherences_aud)
     %   coherence_success_rate(3,c) = coherence_rew_numbers(3,c)/((AVInfo.cohFreq_left_aud(2,c))-(coherence_rew_numbers(4,c)));  %Subtract the trials where there was no chance for reward(N/A Target Correct)
        coherence_success_rate(3,c) = coherence_rew_numbers(3,c)/(AVInfo.cohFreq_left_aud(2,c));  %Subtract the trials where there was no chance for reward(N/A Target Correct)
