%function [] = posthumous_plotA_V_AV(AUD_dataout,VIS_dataout,AV_dataout)
Path = '/Users/adrianaschoenhaut/Documents/AV_Motion_Discrimination_Experiment/AUD_Only_Experiment/test_data/Ba_av/' ;% wherever you want to search
save_name='x';
...
dataout=master_dataout{1};
dataout(strcmp(dataout(:,5),'Yes'),:)=[];
AV_dataout(strcmp(AV_dataout(:,5),'Yes'),:)=[];
VIS_dataout(strcmp(VIS_dataout(:,5),'Yes'),:)=[];
AUD_dataout(strcmp(AUD_dataout(:,5),'Yes'),:)=[];
...
master_dataout=cell(1,3);
master_dataout{1}=AUD_dataout;
master_dataout{2}=VIS_dataout;
master_dataout{3}=AV_dataout;
num_catch_trials=0;
