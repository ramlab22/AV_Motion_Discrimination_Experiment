function [random_dir_list] = dir_randomizer_MCS_AV(AVInfo)
...
%locations and catch trial(1 coherence) locations
    ...
    random_dir_list = zeros(1,AVInfo.n_AV_trials);
    %Direction Randomizer
    
    for c = AVInfo.coherences_aud

        if length(AVInfo.dirSet) > 1 %If we have more than 1 direction to choose from

            numberOfElements = sum(AVInfo.aud_random_coh_list == c); %Sums all of the same coherences from the random list 
            ...
            indeces = find(AVInfo.aud_random_coh_list == c); 
            
            for i = 1:numberOfElements
                if signal(i) == 0
                     random_dir_list(indeces(i)) = AVInfo.dirSet(2); %Leftward wherever that coherence appears 
                elseif signal(i) == 1
                    random_dir_list(indeces(i)) = AVInfo.dirSet(1); %Rightward wherever that coherence appears 
                end

            end
        else
            random_dir_list(:) = AVInfo.dirSet;
        end
    end
