...
waitframes = 1;
refresh_rate = 1/ifi;
rng('shuffle'); % sets the random # seed to current time, making the random number sequence as unpredictable as possible
...
[ExpInfo, vstruct, dotInfo, audInfo, AVInfo] = CreateClassStructure_MCS_AV(monWidth, viewDist, xCenter, yCenter);
    disp(ExpInfo)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
output_counter = 0;
correct_counter = 0;
aud_correct_counter = 0; %initialize counter for fixation on correct target for aud trials
correct_counter2 = 0;
incorrect_counter2=0; %initialize counter for fixation on incorrect target so can end trial when fixtime is reached
block_counter = 1;
gate_off_time = .1;
total_blocks = 1; 
...
%initialize individual modality trial counters for indexing within separate
%modality lists after randomly selecting modality 
audtrial_counter = 0;
vistrial_counter = 0;
AVtrial_counter = 0;
...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    ...
    while (trialcounter <= total_trials) && (BreakState ~= 1) % each trial
        ...
        %Initilize the modality, coherence, and direction for each trial
        ExpInfo.modality = ExpInfo.trial_modality_list{trialcounter};
