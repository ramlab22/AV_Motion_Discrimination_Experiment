function [slope_variability, std_gaussian_variability,slope_sd,std_gaussian_sd] = jackknifeTaskPerformance(slope_per_date, std_gaussian_per_date)
...
n_sessions = length(slope_per_date); % Number of sessions
...
slope_jackknife = zeros(1, n_sessions);
std_gaussian_jackknife = zeros(1, n_sessions);
% slope_at_50_percent_jackknife = zeros(1, n_sessions);

% Perform jackknife resampling for each parameter
for i_session = 1:n_sessions
    % Exclude the i-th session for slope
    slope_subset = slope_per_date([1:i_session-1, i_session+1:end]);
    slope_jackknife(i_session) = mean(slope_subset);
    
    % Exclude the i-th session for standard deviation
    std_gaussian_subset = std_gaussian_per_date([1:i_session-1, i_session+1:end]);
    std_gaussian_jackknife(i_session) = mean(std_gaussian_subset);
    
    % % Exclude the i-th session for slope at 50% threshold
    % slope_at_50_percent_subset = slope_at_50_percent_per_date([1:i_session-1, i_session+1:end]);
    % slope_at_50_percent_jackknife(i_session) = mean(slope_at_50_percent_subset);
end

% Calculate the jackknife estimate of variability (standard error) for each measure
slope_variability = sqrt((n_sessions-1)/n_sessions * sum((slope_jackknife - mean(slope_jackknife)).^2));
std_gaussian_variability = sqrt((n_sessions-1)/n_sessions * sum((std_gaussian_jackknife - mean(std_gaussian_jackknife)).^2));
% slope_at_50_percent_variability = sqrt((n_sessions-1)/n_sessions * sum((slope_at_50_percent_jackknife - mean(slope_at_50_percent_jackknife)).^2));
