...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    ...
    while (trialcounter <= total_trials) && (BreakState ~= 1) % each trial
       ...
       if n_speakers==2
        ...
        [audInfo.CAM, speaker1_5_noise, speaker4_8_noise, speaker3_7_noise] = makeCAM_and_NOISE_8speaker(audInfo.coh, audInfo.dir, audInfo.set_dur, 0, sampling_rate, dB_noise_reduction,n_speakers);
       end
    ...
    %  [CAM_1_Cut_Ramped, CAM_2_Cut_Ramped, audInfo.window_duration] = aud_receptive_field_location(CAM_1,CAM_2, audInfo.t_start, audInfo.t_end,1); 
% 
