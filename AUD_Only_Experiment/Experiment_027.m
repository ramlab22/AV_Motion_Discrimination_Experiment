...
while (BreakState ~= 1) && (block_counter <= total_blocks) % each block
    ...
    while (trialcounter <= total_trials) && (BreakState ~= 1) % each trial
     ...
     %  [CAM_1_Cut_Ramped, CAM_2_Cut_Ramped, audInfo.window_duration] = aud_receptive_field_location(CAM_1,CAM_2, audInfo.t_start, audInfo.t_end, sampling_rate, audInfo.ramp_dur); 
